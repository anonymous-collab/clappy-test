/* ============================================
   CLAPPY - Character Logic v2
   Smooth drag + SVG body + animations
   ============================================ */
...
<svg id="clappy-svg" viewBox="0 0 90 130" xmlns="http://www.w3.org/2000/svg">
  ...
  <g id="clappy-eye-left" style="transform-origin: 33px 68px">
    ...
    <circle id="clappy-pupil-left" cx="33" cy="68" r="4.5" fill="#1a1a1a"/>
  ...
  <g id="clappy-eye-right" style="transform-origin: 57px 68px">
    ...
    <circle id="clappy-pupil-right" cx="57" cy="68" r="4.5" fill="#1a1a1a"/>
...
function moveSinglePupil(pupilEl, eyeId, baseX, baseY, cx, cy) {
  ...
  const dist  = 2.5;
  pupilEl.setAttribute('cx', baseX + Math.cos(angle) * dist);
  pupilEl.setAttribute('cy', baseY + Math.sin(angle) * dist);
}
...
function startDrag(x, y) {
  ...
  clappyRoot.style.transition = 'none';
}
...
function doDrag(x, y) {
  ...
  clappyRoot.style.position  = 'fixed';
  clappyRoot.style.left      = '0px';
  clappyRoot.style.top       = '0px';
  clappyRoot.style.right     = 'auto';
  clappyRoot.style.bottom    = 'auto';
  clappyRoot.style.transform = `translate(${newX}px, ${newY}px)`;
}
...
function endDrag() {
  ...
  clappyRoot.style.transition = 'transform 0.15s ease-out';
}
